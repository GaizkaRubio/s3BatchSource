...
import s3.common.SourceInputFormatProvider;
import s3.common.JobUtils;
import s3.format.CombineTextInputFormat;
import s3.format.FileFormat;
import s3.format.RegexPathFilter;
import s3.format.EmptyInputFormat;
...
public abstract class AbstractFileSource<T extends PluginConfig & FileSourceProperties>
  ...
  public void prepareRun(BatchSourceContext context) throws Exception {
    ...
    LOG.info("prueba: " + pathFileSystem.getLength(path));
    LOG.info("test: " + pathFileSystem.getLength(path));
    S3AFileSystem s3AFileSystem = (S3AFileSystem)pathFileSystem;
    //LOG.info("Metadata from hadoop-aws: ");
    //s3AFileSystem.getObjectMetadata(path).getRawMetadata().forEach((key, value) -> LOG.info(key + ": " + value));
    LOG.info("List Buckets: ");
    //listBuckets();
    LOG.info("Get File Status: ");
    FileStatus file = pathFileSystem.getFileStatus(path);
    LOG.info("" + file.getPath());
    LOG.info("Len: " + file.getLen());
    LOG.info("List Status: ");
    Arrays.stream(pathFileSystem.listStatus(path)).forEach(sta -> LOG.info("" + sta.getPath()));
    Arrays.stream(pathFileSystem.listStatus(path)).forEach(sta -> LOG.info("Len: " + sta.getLen()));
    LOG.info("FileSystem class: " + pathFileSystem.getClass());
    LOG.info("FileSystem: " + pathFileSystem);
    LOG.info("Reading glob status");
  ...
  private void listBuckets() {
    ...
    LOG.info("Metadata New library: ");
    //s3Client.listBuckets().forEach(bucket -> System.out.println(bucket.getName()));
    s3Client.getObjectMetadata("dynamo", "cities_1.csv")
        .getRawMetadata().forEach((key, value) -> LOG.info(key + ": " + value));
    LOG.info("Content-Length" + s3Client.getObjectMetadata("dynamo", "cities_1.csv").getRawMetadata().containsKey("Content-Length"));
    LOG.info("content-length" + s3Client.getObjectMetadata("dynamo", "cities_1.csv").getRawMetadata().containsKey("content-length"));
